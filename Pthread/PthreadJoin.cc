...
{
    int detachstate;
    PthreadAttr::_pthread_attr_getdetachstate(attr, &detachstate);
...
void PthreadJoinManager::GetAttr(pthread_t thread, pthread_attr_t* attr) 
{
    if (IsJoinable(thread)) 
    {
        PthreadAttr::_pthread_attr_setdetachstate(attr, PTHREAD_CREATE_JOINABLE);
    }
    else 
    {
        PthreadAttr::_pthread_attr_setdetachstate(attr, PTHREAD_CREATE_DETACHED);
    }
}
