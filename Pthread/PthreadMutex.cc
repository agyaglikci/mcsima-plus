...
int PthreadMutex::_pthread_mutex_destroy(pthread_mutex_t* mutex) 
{
...
int PthreadMutex::_pthread_mutex_init(pthread_mutex_t* mutex, const pthread_mutexattr_t* mutexattr) 
{
    ASSERTX(mutex != NULL);
    if (mutexattr != NULL) 
    {
        PthreadMutexAttr::_pthread_mutexattr_getkind(mutexattr, &(mutex->__data.__kind));
    }
    else 
    {
        mutex->__data.__kind = PTHREAD_MUTEX_FAST_NP;
    }
    mutex->__data.__count = 0;
    mutex->__data.__owner = -1;
    return 0;
}
...
{
    ASSERTX(mutex != NULL);
    ASSERTX((mutex->__data.__count) >= 0);
    return ((mutex->__data.__count) > 0);
}
...
void PthreadMutex::Lock(pthread_mutex_t* mutex, pthread_t owner) 
{
    ...
    mutex->__data.__owner = (int)owner;
}

void PthreadMutex::LockAgain(pthread_mutex_t* mutex) 
{
    ASSERTX(mutex != NULL);
    ASSERTX(mutex->__data.__kind == PTHREAD_MUTEX_RECURSIVE_NP);
    ASSERTX(mutex->__data.__count >= 1);
...
{
    ASSERTX(mutex != NULL);
    return (pthread_t)mutex->__data.__owner;
}

int PthreadMutex::GetKind(pthread_mutex_t* mutex) 
{
    ASSERTX(mutex != NULL);
    if (mutex->__data.__kind == PTHREAD_MUTEX_DEFAULT) 
    {
        return PTHREAD_MUTEX_FAST_NP;
    }
    else 
    {
        ASSERTX((mutex->__data.__kind == PTHREAD_MUTEX_FAST_NP) ||
                (mutex->__data.__kind == PTHREAD_MUTEX_RECURSIVE_NP) ||
                (mutex->__data.__kind == PTHREAD_MUTEX_ERRORCHECK_NP));
